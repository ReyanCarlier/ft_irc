...
void	main_loop(Server &server, fd_set &readfds, fd_set &writefds) {
	int	_read;
	...
	if (select(server.getHighestFd(&readfds, &writefds) + 1, &readfds,
	&writefds, NULL, NULL) < 0 && errno != EINTR)
